...
ki          = 2 #k is the number on the epsilon-neighborhood criterion (initial)
kf          = 20 #k is the number on the epsilon-neighborhood criterion (final)
...
for k in range(ki,kf+1):
    ...
    if save:
        plotName = ' epsilon-' + str(int(round(epsilon))) + ' k-' + str(k)
        pp.saveWithEpsilonNeighbour(plotName, centerPointsPython, outlierPointsPython, borderPointsPython,
                                    neighbourEdge_points)