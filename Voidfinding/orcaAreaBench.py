...
def run(epsilon, k, file, gen, save, printProgress):
    ...
    outputFile = file.split('/')[2]

    output1 = open("VoidPoints"+outputFile[5:],"w")
    ...
    for t in tri.simplices:
        ...
        if (edges_added < 3):
            output1.write(str(a[0]) + " " + str(a[1]) + "\n")
            output1.write(str(b[0]) + " " + str(b[1]) + "\n")
            output1.write(str(c[0]) + " " + str(c[1]) + "\n")
...
if __name__ == '__main__':
    ...
    run(
        epsilon=30,
        k=20,
        file='Data/irr_bench/20irr2d_100000.dat',
        gen=5,
        save=False,
        printProgress=False
    )